...
#define GET_OP(expr) (FIRST(expr)->value)
...
static void *k_dup(const void *k) { return strdup(k); }
...
static int eval_binary(AST *expr, HTable *vars, int32_t *value)
{
  ...
  if (!strcmp(GET_OP(expr), "+"))
    *value = lhs + rhs;
  else if (!strcmp(GET_OP(expr), "-"))
    *value = lhs - rhs;
  else if (!strcmp(GET_OP(expr), "*"))
    *value = lhs * rhs;
  else if (!strcmp(GET_OP(expr), "/"))
...
static int execute_block(AST *block, const char *prev_block, HTable *vars, const char **next_block)
{
...
static int interpret_cfg(AST *blocks, int32_t *result)
{
  ...
  if (register_blocks(blocks, block_tab, &entry_block) == 0)
  {
    ...
    while (current_block)
    {
      ...
      if (execute_block(current_block, prev_block, vars, &next_label) < 0)
        break;
      if (!next_label)
      {
        status = 0;
  ...
  if (!status)
  {
    int32_t *stored_result = NULL;
    if (!ht_get(vars, "result", (void **)&stored_result))
    {
      fprintf(stderr, "Fatal: no result in the program\n");
      status = -1;
    }
    else
...
int interpret(AST *program)
{
  ...
  status = interpret_cfg(program->lchild->lchild, &value);
  if (!status)
    printf("Result: %d\n", value);
  D_AST(program);
  return status;
