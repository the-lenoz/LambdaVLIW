...
int compile_expr(AST *expr, int result_SSA_name, SSA_Func *f)
{
  ...
  if (expr->type == SEXP)
  {
    ...
    STR_CASE("let")
    compile_expr(expr->rchild->lchild->rchild->lchild, );
...
int compile_program(AST *program, FILE *out_fp)
{
  ...
  int status = compile_expr(CAR(CAR(program)), "res", out_fp, &var_name_c);
